...
pub unsafe fn main() -> i32 {
    ...
    let dylib_path = Path::new(dylib_path.trim())
        .absolutize_from(&exec_dir)
        .unwrap();

    let embedder_lib = libloading::Library::new(dylib_path.as_ref()).unwrap();
    let embedder_main: libloading::Symbol<unsafe extern "C" fn(*mut c_void) -> i32> =
        embedder_lib.get(b"node_embedder_main").unwrap();
